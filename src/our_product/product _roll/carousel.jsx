...
export default function Carousel() {
  ...
  const handleLeftClick = () => {
    ...
    const activeIndex = prevState.findIndex((card) => card.active);
    const nextIndex = activeIndex === 0 ? prevState.length - 1 : activeIndex - 1;
    prevState.forEach((card, index) => {
      card.active = index === nextIndex;
    });
    setCards(prevState);
  };
  ...
  const handleRightClick = () => {
    const prevState = [...cards];
    const activeIndex = prevState.findIndex((card) => card.active);
    const nextIndex = activeIndex === prevState.length - 1 ? 0 : activeIndex + 1;
    prevState.forEach((card, index) => {
      card.active = index === nextIndex;
    });
    setCards(prevState);
  ...
  return (
    <>
      <div className="flex flex-row-3 gap-48 justify-center">
        <div
        ...
        {cards.map((card, index) => (
          <ProductCard key={index} prop={card.text} isActive={card.active} />
        ))}
        <div
          className="text-xl md:text-5xl cursor-pointer"
...
//             .sort((a, b) => (a.pos > b.pos ? 1 : b.pos > a.pos ? -1 : 0))
//             .map((card, index) => (
//               <Card key={index} prop={card.text} style={index === 1 ? { fontSize: "2rem" } : {}} />
//             ))}
//           <div
//             className="text-xl md:text-5xl cursor-pointer"
//             onClick={() => handleRightClick()}
